...
if (!canImport) {
  ...
  const unmatchedAgentNames =
    new Map<string, number>();

  const preview = rows.map(
    (row, index) => {
      const accountCode = clean(
        field(row, "A/C")
      ).toUpperCase();
      ...
      const tradeContact = clean(
        field(row, "TRADE CONTACT")
      );

      const buyingGroup = clean(
        field(row, "ANALYSIS1")
      );

      const creditLimit =
        numberValue(
          field(row, "CREDIT LIMIT")
        );
      ...
      if (
        ...
        return {
          ...
          status: "INVALID",
          changes: [
            "Missing account code or customer name",
          ],
        };
      }
